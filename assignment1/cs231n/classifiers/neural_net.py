...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # Backprop through scores ops.
    dscores = p
    dscores[range(N),y] -= 1
    dscores /= N
    
    # Backprop through h1.dot(W2) + b2 layer.
    grads["W2"] = np.dot(h1.T, dscores)
    grads["b2"] = np.sum(dscores, axis=0)#, keepdims=True)
    dhidden = np.dot(dscores, W2.T)
    
    # Backprop through np.maximum(0, X.dot(W1) + b1) layer
    dhidden[h1 <= 0] = 0
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in range(num_iters):
      ...
      for (key, val) in grads.items():
        #print(key, val.shape, self.params[key].shape)
